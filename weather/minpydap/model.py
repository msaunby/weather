...
from minpydap.lib import quote, walk, fix_slice
from minpydap.util.odict import odict
...
class SequenceType(StructureType):
    ...
    def __getitem__(self, key):
        ...
        if isinstance(key, basestring) and key in self:
            ...
            if isinstance(key, tuple):
                ...
                out._keys = list(key)
                if out._data is not None: out.data = out.data[key]  # SequenceData protocol
            else:
    ...
    def __iter__(self):
        ...
        data = [el for el in data]  # download data before extracting columns
        cols = (get_row(data, i, self._nesting_level) for
                i, k in enumerate(self.keys()))
        for row in zip(*cols):
